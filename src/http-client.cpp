...
bool HttpClient::send()
{
	...
	if (curl) {
		...
		for (HttpClientFormField &form_field : this->form_fields) {
			...
			if (form_field.data != NULL)
				curl_mime_data(field, form_field.data,
					       form_field.datasize > 0 ? form_field.datasize : CURL_ZERO_TERMINATED);
		}

		if (this->form_fields.size() > 0)
			curl_easy_setopt(curl, CURLOPT_MIMEPOST, form);

		//obs_log(LOG_DEBUG, "try to send http request to %s (method: %s)", url.c_str(),
			HTTP_CLIENT_METHOD_STR[this->method]);
		this->error.curl_code = curl_easy_perform(curl);
		//obs_log(LOG_DEBUG, "  curl_code           : %d", (int)this->error.curl_code);

		curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &this->error.http_code);
		//obs_log(LOG_DEBUG, "  http_code           : %d", (int)this->error.http_code);
		...
		if (this->error.curl_code == CURLE_OK) {
			...
			//obs_log(LOG_DEBUG, "  response is         : %s", this->response_obs_data ? "json" : "not json");

			if (this->response_obs_data) {
				bool has_success_item = obs_data_has_user_value(this->response_obs_data, "success");
				//obs_log(LOG_DEBUG, "  has_success_item    : %s", has_success_item ? "yes" : "no");
				if (has_success_item)
					this->success_json_item = obs_data_get_bool(this->response_obs_data, "success")
				...
				//obs_log(LOG_DEBUG, "  has_error_item      : %s", has_error_item ? "yes" : "no");
				if (has_error_item)
					this->error_json_item = obs_data_get_string(this->response_obs_data, "error");
			}
		}

		//obs_log(LOG_DEBUG, "  success_json_item   : %s", HTTP_CLIENT_BOOL_ITEM_STR[this->success_json_item]);
		//obs_log(LOG_DEBUG, "  error_json_item     : %s", this->error_json_item.c_str());

		if (this->error.curl_code == CURLE_OK && this->error.http_code == 200 &&
		    this->success_json_item != HTTP_CLIENT_BOOL_ITEM_FALSE && this->error_json_item.empty())
	...
	//obs_log(LOG_DEBUG, "  errorStr            : %s", this->error.error.c_str());
	//obs_log(LOG_DEBUG, "  send_success        : %s", this->send_success ? "TRUE" : "FALSE");
