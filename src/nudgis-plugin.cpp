...
NudgisSettings::NudgisSettings() : QWidget(nullptr), ui(new Ui_Settings)
{
	...
	for (QComboBox *combo_auto_state : combos_auto_state) {
		for (const AutoState &autostate : AutoState::GetAll())
			combo_auto_state->addItem(obs_module_text(autostate.name.c_str()),
...
void NudgisSettings::loadSettings(const NudgisConfig *nudgis_config)
{
	...
	ui->auto_delete_uploaded_file->setCurrentIndex(
		ui->auto_delete_uploaded_file->findData(QVariant::fromValue(nudgis_config->auto_delete_uploaded_file)));
	ui->publish_recording_automatically->setCurrentIndex(ui->publish_recording_automatically->findData(
		QVariant::fromValue(nudgis_config->publish_recording_automatically)));
}
...
NudgisConfig &NudgisSettings::to_NudgisConfig()
{
	...
	nudgis_config.auto_delete_uploaded_file =
		ui->auto_delete_uploaded_file->currentData().value<const AutoState *>();
	nudgis_config.publish_recording_automatically =
		ui->publish_recording_automatically->currentData().value<const AutoState *>();
...
void NudgisSettings::on_btn_test_clicked()
{
	...
	obs_frontend_push_ui_translation(obs_module_get_string);
	NudgisTestParamsUi nudgis_test_params_ui{this, &nudgis_test_params_data,
...
static void obs_event(enum obs_frontend_event event, void *private_data)
{
	...
	if (event == OBS_FRONTEND_EVENT_RECORDING_STOPPED) {
		...
		if (recording_output != NULL) {
			...
			if (settings != NULL) {
				if (NudgisConfig::GetCurrentNudgisConfig()->publish_recording_automatically->type !=
					...
					obs_frontend_push_ui_translation(obs_module_get_string);
					NudgisUploadUi nudgis_upload_ui((QWidget *)obs_frontend_get_main_window(),
									path);
					obs_frontend_pop_ui_translation();
					nudgis_upload_ui.exec();

					obs_log(LOG_INFO, "nudgis_upload_ui->show pass");
				}
...
{
	obs_log(LOG_INFO, "Nudgis plugin loaded successfully (version %s)", PLUGIN_VERSION);
	...
	nudgis_service =
		obs_service_create(nudgis_service_info.id, nudgis_service_info.get_name(nullptr), nullptr, nullptr);
